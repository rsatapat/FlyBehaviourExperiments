...
while (trials < 60):
    ...
    stimulation_time = random.choice([0.2, 0.5, 1])
    see_fly = 0
    # t = time.time()
    image_list = []
    while(True):
        ...
        this, contours, hierarchy = cv2.findContours(diff_img, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        pos, ellipse, cnt = fly_track.fly_court_pos(contours, size_cutoff=400, max_size=7000)
        ...
        if len(pos) != 1:
            ...
            if see_fly == 0:
                if pos[0][0] > 48 and pos[0][0] < 736-48 and pos[0][1] > 48 and pos[0][1] < 736-48:
                    see_fly = 1
                    frame_number = 0
                    print('fly_detected')
            else:
        ...
        if see_fly == 1:
            if frame_number == 0:
                ...
                stim = random.choice([1])
                if stim == 1:
                    # print('1')
                    ard_ser.write(b'L')
                else:
                    ard_ser.write(b'H')
            elif frame_number > framerate and frame_number < (stimulation_time) * framerate:
                pass
            elif frame_number == (stimulation_time) * framerate:
                stim = 0
                ard_ser.write(b'H')
            elif frame_number > (stimulation_time+1) * framerate:
                frame_number = -5000
                see_fly=0
                stim = 0
                print('Stimulation off...')
                stimulus_inst = objects.stim_inst([0], 0, 0,stimulation_time,
                                              time.clock() - time_exp_start,
                                              time.clock() - time_stim_start,
                                              frame_number, video_frame, 0, 0)
                ...
                for images in image_list:
                    writer_small.writeFrame(images)
                del image_list
                time.sleep(10)
                break

            # writer_small.writeFrame(cv_image)
            image_list.append(cv_image)
            writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp - t1, stim])

        # rect_stim.draw()
        # win.flip()
        # writer_csv.writerow(rect_stim.fillColor)

        # writer_small.writeFrame(cv_image)
        # cropped_image = cv_image[int(pos[0][1]) - 50:int(pos[0][1]) + 50, int(pos[0][0]) - 50:int(pos[0][0]) + 50]
        # if cropped_image.shape == (100, 100):
