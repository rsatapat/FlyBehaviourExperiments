...
writer_csv.writerow(['pos_x', 'pos_y', 'fly_frame_ori', 'timestamp', 'stim', 'LED color'])
...
while (trials < 90):
    ...
    see_fly = 0
    # stimulation_time = random.choice([0.2, 0.5, 1, 2])   #in seconds
    stimulation_time = random.choice([1.5])  # in seconds
    on_time = 10
    ...
    while(True):
        ...
        this, contours, hierarchy = cv2.findContours(diff_img, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        pos, ellipse, cnt = fly_track.fly_court_pos(contours, size_cutoff=100, max_size=7000)
        ...
        if see_fly == 1:
            ...
            if frame_number == 0:
            ...
            elif frame_number == int(1 * framerate):
                ...
                pass
            elif frame_number == int((1+stimulation_time) * framerate):
                stim = 0
                ard_ser.write(b'H')
            elif frame_number > (stimulation_time+1+1) * framerate:
                frame_number = -5000
                see_fly=0
                stim = 0
                print('Stimulation off...')
                stimulus_inst = objects.stim_inst([color], on_time, frequency, stimulation_time,
            ...
            writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp - t1, stim, color])
