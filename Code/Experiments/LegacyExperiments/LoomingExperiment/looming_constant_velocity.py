...
class loom_stim:
    ...
    def angle_subtended(self,frames):
        return (self.size)/((self.distance-self.speed*(frames/(1*60))))

    def radius_in_px(self):
        result = []
        for i in range(int(self.toi*60)):
            result.append((self.angle_subtended(i) * 2 * (1024/60)))
        return result
...
while(trials<40):
    frames_lost = 0
    loom_stim_speed = random.choice([250])
    loom_stim_size = random.choice([10,20])
    loom_stim_1 = loom_stim(loom_stim_speed,loom_stim_size,120,0.5)
    size_list = loom_stim_1.radius_in_px()
    for i in range(30):
        if size_list[i] < 0:
            size_list[i:] = [size_list[i - 1]] * (30 - i)
    print(len(size_list))
    frame_number = 0
    offset_x = 0
    offset_y = 0
    # offset_x = random.choice(np.linspace(-20, 20, 21))
    # offset_y = random.choice(np.linspace(-20, 20, 21))
    while(True):
        # image, timestamp = image_methods.grab_image(camera, 'ptgrey')
        image, timestamp = image_methods.grab_image(camera, 'basler')
        ...
        if len(pos) != 1:
            ...
            print('fly lost')
            pos.append(position)
            if frames_lost > 600:  # if the number of lost frames is more than 15, abort experiment
        ...
        if frame_number < 570:
        ...
        elif frame_number >= 570 and frame_number < 30+570:
            if frame_number == 570:
            ...
            #if Hz != 50:
            dot_stim.radius = size_list[frame_number - (570)]/512
            # dot_stim.radius = 1
            # print(dot_stim.radius, frame_number)
            dot_stim.pos = [[x,y]]
            # dot_stim.pos = [[0, 0]]
            #else:
            #    dot_stim.radius = 0.5
            ...
            dot_stim.draw()
            win.flip(clearBuffer=True)  # slowest part of the algorithm,takes ~10ms
            color = dot_stim.fillColor[0]

        elif frame_number > 30+570 and frame_number < 30+30+570:
            # print(dot_stim.radius, frame_number)
            dot_stim.pos = [[x, y ]]
            dot_stim.draw()
            win.flip(clearBuffer=True)  # slowest part of the algorithm,takes ~10ms
        ...
        elif frame_number >= 30+30+570:
            stimulus_inst = objects.stim_inst([stimulus,seed,offset_x,offset_y], trials, loom_stim_speed,loom_stim_size,
                                              time.clock() - time_exp_start,
                                              time.clock() - time_stim_start,
                                              frame_number, video_frame, 0, 2)
            ...
            trials = trials + 1
            break
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp, color])
