...
while(trials<400):
    ...
    frame_time = 60 * random.choice(num_bars)
    print(Hz, frame_time)
    frame_number = 0
    offset_x = 0
    offset_y = 0
    while(True):
        ...
        if frame_number >= 600 and frame_number < frame_time+600:
            if frame_number == 600:
                ...
                temp_x = x
                temp_y = y
                color = random.choice([-1,-0.5,-0.75,0.5,1])
...
win.saveFrameIntervals(fileName = filename+'.log',clear = True)
