...
class loom_stim:
    ...
    def angle_subtended(self,frames):
        return (self.size/2)/(2*(self.distance-self.speed*(frames/(self.toi*60))))
...
while(trials<180):
    ...
    while(True):
        ...
        if frame_number < 600:
            ...
            stim_color = dot_stim.fillColor[0]

        if frame_number >= 600 and frame_number < frame_time+600:
            if frame_number == 600:
                offset_x = random.choice(np.linspace(-20,20,21))
                offset_y = random.choice(np.linspace(-20,20,21))
                temp_x = x
                temp_y = y
                color = np.random.choice([-1, 1, 2, -0.5])
                print(color)
                dot_stim.fillColor = (color, color, color)
            wheel_stim.size = wheel_stim.size + (Hz/512)
            dot_stim.radius = dot_stim.radius + (Hz/512)
            wheel_stim.pos = [[x+(offset_x/512), y+(offset_y/512)]]
        ...
        if frame_number >= frame_time+600:
            stimulus_inst = objects.stim_inst([stimulus,seed,offset_x,offset_y], trials, frame_time, Hz,
                                              time.clock() - time_exp_start,
                                              time.clock() - time_stim_start,
                                              frame_number, video_frame, 0, 2)
            stimulus_inst.stim_attributes = [color]
            ...
            trials = trials + 1
            break
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp, stim_color])
