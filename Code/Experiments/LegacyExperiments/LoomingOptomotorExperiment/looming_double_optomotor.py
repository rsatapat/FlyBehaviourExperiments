...
"""
First, a loom appears, then a dot looms over the ring-pinwheel and then, the pinwheel starts rotating
"""

class loom_stim:
    def __init__(self,speed,size,distance,toi):
        self.speed = speed
        self.size = size
...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp', 'direction', 'loom_status'])
...
stimulus_image = '24ringpinwheel_100_6.png'
...
while (trials < 120):
    ...
    closedloop_gain = 0
    loom_speed = random.choice([4]) ## DO NOT CHANGE THIS, TO ENSURE THAT THE LOOMING STIMULUS NEVER
                                    ## INTERACTS WITH THE PINWHEEL
    color = random.choice([-0.5,-0.65,-0.75,-0.9,-1])
    loom_time = random.choice([2,4,6])
    inter_loom_time = random.choice([0.2,0.4,0.8,1.6])
    ...
    while (True):  # this unit(of experiment) is repeated #
        ...
        if frame_number >= 0 * framerate and frame_number < loom_time * framerate:
            dir = 0
        ...
        elif frame_number >= loom_time * framerate and frame_number < (loom_time+0.4) * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (color, color, color)
            dot_stim.radius = dot_stim.radius + (loom_speed / 512)
            dot_stim.draw()
        elif frame_number >= (loom_time+0.4) * framerate and frame_number < (loom_time + inter_loom_time + 0.4) * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
            dot_stim.radius = 1/512
            dot_stim.draw()
        elif frame_number >= (loom_time + inter_loom_time + 0.4) * framerate and frame_number < (loom_time + inter_loom_time + 0.4 + 0.4) * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (color, color, color)
            dot_stim.radius = dot_stim.radius + (loom_speed / 512)
            dot_stim.draw()
        elif frame_number >= (loom_time + inter_loom_time + 0.4 + 0.4)  * framerate and frame_number < 10 * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
        ...
        elif frame_number >= 10 * framerate and frame_number < (10+loom_time) * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
            dot_stim.radius = 1/512
            dot_stim.draw()
        elif frame_number >= (10+loom_time) * framerate and frame_number < (10+loom_time + 0.4) * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (color, color, color)
            dot_stim.radius = dot_stim.radius + (loom_speed / 512)
            dot_stim.draw()
        elif frame_number >= (10+loom_time + 0.4) * framerate and frame_number < (10+loom_time + inter_loom_time + 0.4) * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
            dot_stim.radius = 1/512
            dot_stim.draw()
        elif frame_number >= (10+loom_time + inter_loom_time + 0.4) * framerate and frame_number < (10+loom_time + inter_loom_time + 0.4 + 0.4) * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (color, color, color)
            dot_stim.radius = dot_stim.radius + (loom_speed / 512)
            dot_stim.draw()
        elif frame_number >= (10+loom_time + inter_loom_time + 0.4 + 0.4) * framerate and frame_number < 20 * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
        ...
        elif frame_number >= 20 * framerate and frame_number < (20 + loom_time) * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            # wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
            dot_stim.radius = 1 / 512
            dot_stim.draw()
        elif frame_number >= (20 + loom_time) * framerate and frame_number < (20 + loom_time + 0.4) * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            # wheel_stim.draw()
            dot_stim.fillColor = (color, color, color)
            dot_stim.radius = dot_stim.radius + (loom_speed / 512)
            dot_stim.draw()
        elif frame_number >= (20 + loom_time + 0.4) * framerate and frame_number < (20 + loom_time + inter_loom_time + 0.4) * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            # wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
            dot_stim.radius = 1/512
            dot_stim.draw()
        elif frame_number >= (20 + loom_time + inter_loom_time + 0.4) * framerate and frame_number < (20 + loom_time + inter_loom_time + 0.4 + 0.4) * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            # wheel_stim.draw()
            dot_stim.fillColor = (color, color, color)
            dot_stim.radius = dot_stim.radius + (loom_speed / 512)
            dot_stim.draw()
        elif frame_number >= (20 + loom_time + inter_loom_time + 0.4 + 0.4) * framerate and frame_number < 30 * framerate:
            dir = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            # wheel_stim.draw()
            dot_stim.fillColor = (0, 0, 0)
        ...
        elif frame_number >= 30 * framerate:
            ...
            stimulus_inst.stim_attributes = [wheel_stim.mask,closedloop_gain, 'loom', loom_speed]
        ...
        loom_status = dot_stim.fillColor
