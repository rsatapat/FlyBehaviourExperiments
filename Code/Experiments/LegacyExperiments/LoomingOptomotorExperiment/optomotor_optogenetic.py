...
stimulus_image = 'ringpinwheel16_blue100_6.png'
...
rect_stim = psychopy.visual.Rect(
    win=win,
    units='norm',
    width=4,
    height = 2,
...
while (trials < 60):
    ...
    closedloop_gain = 0
    # loom_speed = random.choice([4])
    # color = random.choice([-0.6, -0.65, -0.7,-0.75,-0.8])
    # loom_time = random.choice([2, 4, 6])
    color = random.choice([(1, -1, -1), (-1, -1, 1)])
    # color = 1
    # stimulation_time = random.choice([0.1,0.5,1,5])
    ...
    while (True):  # this unit(of experiment) is repeated #
        ...
        dot_stim.pos = [[x, y]]
        wheel_stim.pos = [[x, y]]
        # if frame_number >= 0 * framerate and frame_number < stimulation_time * framerate:
        #     print('Stimulation on..')
        #     rect_stim.fillColor = color
        #     rect_stim.draw()
        # elif frame_number > 0*framerate and frame_number < stimulation_time * framerate:
        #     rect_stim.fillColor = color
        #     rect_stim.draw()
        #     # dot_stim.fillColor = color
        #     # dot_stim.draw()
        #
        # elif frame_number >= stimulation_time*framerate and frame_number < (stimulation_time + 20) * framerate:
        #     rect_stim.fillColor = -1
        #     rect_stim.draw()
            # dot_stim.fillColor = -1
            # dot_stim.draw()

        if frame_number >= 0 * framerate and frame_number < 10 * framerate:
            dir = direction
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            dot_stim.fillColor = (-1, -1, 1)
            dot_stim.lineColor = (-1, -1, 1)
            dot_stim.radius = 0.15
            dot_stim.draw()
        ...
        # elif frame_number >= (stimulation_time + 20) * framerate:
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp, dir, dot_stim.fillColor])
        # writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp, rect_stim.fillColor])
        # writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp, dot_stim.fillColor])
        ...
        if dot_stim.fillColor[0] == 1:
            cv2.circle(cropped_image, (45,45),10,(0,255,0),-1)
