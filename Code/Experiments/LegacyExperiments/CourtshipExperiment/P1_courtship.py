...
#add paths
import sys
sys.path.append(r'D:\Roshan\Project\PythonCodes\Codes\Experiments\__pycache__')
sys.path.append(r'D:\Roshan\Project\PythonCodes\Codes\Experiments\LegacyExperiments\utils')
# my modules
import Dialog_box
...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp','female_angle','female_size','opto_stim', 'pinwheel_angle'])
...
while(True):
    ...
    # image, timestamp = image_methods.grab_image(camera, 'basler')
    cv_image = ROI(image, loc, size)
    ret, diff_img = cv2.threshold(cv_image, 65, 255, cv2.THRESH_BINARY)
    this, contours, hierarchy = cv2.findContours(diff_img, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
# ard_ser.write(b'L')
t1=0
while (trials < 90): ## number of trials
    # main loop
    ...
    ## dot stimulus parameters
    dist_female = 250
    speed_of_female = random.choice([1.5]) #originally 2, decrease to slow down dot movement
    ...
    dot_amplitude = random.choice([60])
    bias = random.choice([1, 0.5, 0.6, 0.7])
    choice = [1, -1]
    disappear = random.choice([-60, -30, 0, 30, 60])
    static_angle = random.choice([-45, -22.5, 0, 22.4, 45])
    dis_or_not = random.choice([-1, 1, 1, 1, 1, 1])

    ## pinwheel stimulus parameters
    pinwheel_duration = 30 ## in seconds, how long the pinwheel will spin
    trial_duration = 36 ## in seconds, how long the trial will last (including the time when the pinwheel is not spinning)
    only_dot_duration = trial_duration - pinwheel_duration
    Hz = random.choice([5])
    direction = random.choice([1,-1])
    bars = 6
    contrast = random.choice([25, 10, 50])
    stimulus_image_pinwheel = 'sine_pinwheel' + '_' + str(contrast) + '_' + str(bars) + '.png'
    ...
    while (True):
        ...
        ret, diff_img = cv2.threshold(cv_image, 65, 255, cv2.THRESH_BINARY)
        this, contours, hierarchy = cv2.findContours(diff_img, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        pos, ellipse, cnt = fly_track.fly_court_pos(contours, size_cutoff=150, max_size = 6000)
        ...
        if frame_number == 0:
        ...
        elif frame_number >= 0 * framerate and frame_number < only_dot_duration * framerate:  ### the dot is shown for 6 seconds without pinwheel spinning
            stim = 0
            dot_stim.radius = 1/15
            dot_stim.fillColor = -1
            dot_stim.lineColor = dot_stim.fillColor
            ## angular position of dot
            angle = dot_amplitude-abs(2*dot_amplitude - (speed_of_female*frame_number % (4*dot_amplitude)))
            ## here the dot will expand and contract to control speed
            # dot_stim.radius = 1 / 15 + (angle / 3) / 512
            ## here the dot will stay static at an angle wrt the fly
            # angle = static_angle
            ## here the dot moves around by random amounts till it reached the maximum and then goes back to zero
            # angle = angle + speed_of_female * choice[random.random() > bias]
            # if abs(angle) >= 60:
            #     angle = 0
            #     if choice == [1, -1]:
            #         choice = [-1, 1]
            #     elif choice == [-1, 1]:
            #         choice = [1, -1]
            ## compute position and angle for dot and pinwheel
            ...
            ## make the dot disappear for given period number of frames
            # if disappear-(4*speed_of_female) < angle < disappear+(4*speed_of_female):
            #     if dis_or_not == 1:
            #         dot_stim.fillColor = 0
            #         dot_stim.lineColor = dot_stim.fillColor
            #         stim = 0
            #         angle = 999
            ## this is for pinwheel to turn
            wheel_stim.ori = wheel_stim.ori + direction * (360 * Hz) / (framerate * bars) * stim
            # wheel_stim.draw()
            dot_stim.draw()
        elif frame_number >= only_dot_duration * framerate and frame_number < trial_duration * framerate: ### for 30 seconds, the dot is shown with the pinwheel spinning
            stim = 1
            dot_stim.radius = 1/15
            dot_stim.fillColor = -1
            dot_stim.lineColor = dot_stim.fillColor
            ...
            #         dot_stim.lineColor = dot_stim.fillColor
            #         angle = 999
            ## the pinwheel starts spinning and the dot disappears
            if frame_number >= 9 * framerate and frame_number < 10.5 * framerate:
                dot_stim.fillColor = 0
                dot_stim.lineColor = dot_stim.fillColor
                dot_stim.radius = 0
            ## this is for pinwheel to turn
            ...
            ## draw the dot
            dot_stim.draw()
        elif frame_number >= trial_duration * framerate:
            print('Stimulation off...')
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor), direction, Hz, contrast, dis_or_not],on_time, off_time, frequency,
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp-t1, angle, float(dot_stim.radius),stim,-1* float(wheel_stim.ori)])
        cropped_image = cv_image[int(pos[0][1]) - 60:int(pos[0][1]) + 60, int(pos[0][0]) - 60:int(pos[0][0]) + 60]
        if cropped_image.shape == (120, 120):
