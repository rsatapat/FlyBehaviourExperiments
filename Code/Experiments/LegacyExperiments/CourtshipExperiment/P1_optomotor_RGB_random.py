...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp','female_angle','opto_stim','optomotor'])
...
while (trials < 40):
    ...
    dot_stim2.fillColor = random.choice([-1, -0.2])
    while (True):
        image, timestamp = image_methods.grab_image(camera, 'basler')
        cv_image = ROI(image, loc, size)
        ret, diff_img = cv2.threshold(cv_image, 60, 255, cv2.THRESH_BINARY)
        ...
        if frame_number == 0:
            ...
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = 0
            # win.color = random.choice([(-1, 1, -1), (-1, -1, 1)])
            ...
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = 0
        elif frame_number >= 0 * framerate and frame_number < 20 * framerate:
            ...
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor), bars, contrast, Hz, list(win.color)],on_time, off_time, frequency,
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp-t1, angle, stim, dir])
        cropped_image = cv_image[int(pos[0][1]) - 60:int(pos[0][1]) + 60, int(pos[0][0]) - 60:int(pos[0][0]) + 60]
