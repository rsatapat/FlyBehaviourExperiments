...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp','female_angle','opto_stim','optomotor','pinwheel_angle', 'female_x', 'female_y'])
...
stim_size = size_stim
angle_opto = (360 * Hz) / (framerate * bars)
...
while(True):
    ...
    image, timestamp = image_methods.grab_image(camera, 'ptgrey')
    cv_image = ROI(image, loc, size)
    ret, diff_img = cv2.threshold(cv_image, 55, 255, cv2.THRESH_BINARY)
    this, contours, hierarchy = cv2.findContours(diff_img, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
while (trials < 80):
    ...
    ############################
    contrast = random.choice([25, 50, 75, 100])
    # contrast = random.choice([25])
    ...
    frame_lost = 0
    dist_female = 250
    speed_of_female = random.choice([2])
    ...
    # phase_difference = direction
    phase_difference = random.choice([0, 180, 90, -90])
    dot_amplitude = random.choice([45])
    # print(phase_difference)
    # dot_stim.fillColor = random.choice([-1, -0.2, -0.5, -0.8])
    # dot_stim.fillColor = random.choice([0, -0.5, -0.8, -1])
    dot_stim.fillColor = random.choice([-1])
    # Hz = random.choice([5])
    Hz = random.choice([10])
    t1 = 0
    pinwheel_amplitude = (dot_amplitude/speed_of_female) * ((360 * Hz) / (framerate * bars))
    print(phase_difference, pinwheel_amplitude)
    # stimulus_type = random.choice(['random', 'oscillate', 'continuous'])
    stimulus_type = random.choice(['continuous'])
    while (True):
        # image, timestamp = image_methods.grab_image(camera, 'basler')
        image, timestamp = image_methods.grab_image(camera, 'ptgrey')
        cv_image = ROI(image, loc, size)
        ...
        if frame_number == 0:
            ...
            # random
            # angle = angle + speed_of_female * random.choice([1, -1])  ## for random dot stimulus
            # if abs(angle) >= dot_amplitude:
            #     angle = 0
            # sine
            # angle = frame_number * 2 * speed_of_female % 360
            # angle = dot_amplitude * (np.sin((angle / 180) * np.pi))
            optomotor_stim = 0
            dir = 0
            ...
            y_dot = (y_dot - 512) / h_ratio + h_rem
            dot_stim.pos = [[x_dot, y_dot]]
            if stimulus_type == 'oscillate':
                ## for oscillating pinwheel
                # pinwheel_angle = pinwheel_amplitude-abs(2*pinwheel_amplitude - (((360 * Hz) / (framerate * bars))*frame_number % (4*pinwheel_amplitude))) + phase_difference
                # wheel_stim.ori = pinwheel_angle
                ####################
                pinwheel_angle = (frame_number * 2 * speed_of_female % 360) + phase_difference
                pinwheel_angle = pinwheel_amplitude * (np.sin((pinwheel_angle / 180) * np.pi))
                wheel_stim.ori = pinwheel_angle
            elif stimulus_type=='random':
                # wheel_stim.ori = wheel_stim.ori + Hz * random.random() * random.choice([1, -1]) ## for random noise stimulus
                wheel_stim.ori = wheel_stim.ori + ((360 * Hz) / (framerate * bars)) * random.choice([1, -1])  ## for random noise stimulus
            elif stimulus_type == 'continuous':
                wheel_stim.ori = wheel_stim.ori + dir * (360 * Hz) / (framerate * bars)
            #########
            # wheel_stim.draw()
            dot_stim.draw()
        elif frame_number >= 12 * framerate:
            print('Stimulation off...')
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor), bars, contrast, Hz, stimulus_type],on_time, off_time, frequency,
            ...
            break
        win.flip()
        # print(-1*wheel_stim.ori, angle)
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp-t1,angle, stim, phase_difference+180, -1*wheel_stim.ori, x_dot, y_dot])
        cropped_image = cv_image[int(pos[0][1]) - 60:int(pos[0][1]) + 60, int(pos[0][0]) - 60:int(pos[0][0]) + 60]
