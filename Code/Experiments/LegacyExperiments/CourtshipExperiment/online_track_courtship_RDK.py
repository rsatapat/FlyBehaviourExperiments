...
if __name__ == "__main__":
    ...
    writer_csv.writerow(['pos_x_m', 'pos_y_m', 'ori_m', 'pos_x_f', 'pos_y_f', 'ori_f', 'timestamp', 'direction'])
    ...
    N = 200
    # coherence = random.choice([0.5, 0.6, 0.75])
    ...
    while (video_frame < minutes * 60 * framerate):
        ...
        writer_csv.writerow([position[0][0], position[0][1], ori[0], position[1][0], position[1][1], ori[1], timestamp-t1, stim * rod])
        ...
        dot_stim1.fieldPos = [[x_m, y_m]]
        dot_stim2.fieldPos = [[x_m, y_m]]
        if frame_number < stim_time * framerate:
            ...
            STIM_ON_OFF = [0, 1]
            STIM_ON_OFF = [0, 1]
            stimulus_inst = objects.stim_inst([stimulus, courtship_start_point, coherence, rod], direction, N, speed,
