...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp','female_angle','female_angle2','opto_stim'])
...
while (trials < 40):
    ...
    x, y = -1, -1
    stim = 0
    dot_stim2.fillColor = random.choice([0, -0.2, -0.5, -1])
    while (True):
        ...
        if frame_number == 0:
            ...
            dot_stim.pos = [[x, y]]
            dot_stim.draw()
            x, y = int(dist_female2 * np.cos((((fly_frame_ori + angle2) / 180) * np.pi) + np.pi / 2) + pos[0][0]), int(
                dist_female2 * np.sin((((fly_frame_ori + angle2) / 180) * np.pi) + np.pi / 2) + pos[0][1])
            x = (x - 512) / w_ratio + w_rem
            y = (y - 512) / h_ratio + h_rem
            dot_stim2.pos = [[x, y]]
            dot_stim2.draw()
            ...
            stim = 0
            angle = 45-abs(90 - (speed_of_female*frame_number % 180))
            # angle2 = 45-abs(90 - (((speed_of_female2*(frame_number)) % 180)))
            angle2 = angle * -1
            x, y = int(dist_female * np.cos((((fly_frame_ori + angle) / 180) * np.pi) + np.pi / 2) + pos[0][0]), int(
                dist_female * np.sin((((fly_frame_ori + angle) / 180) * np.pi) + np.pi / 2) + pos[0][1])
            x = (x - 512) / w_ratio + w_rem
            ...
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor), float(dot_stim2.radius), list(dot_stim2.fillColor)],on_time, off_time, frequency,
                                          time.clock() - time_exp_start,
                                          time.clock() - time_exp_start,
                                          time.clock() - time_stim_start,
                                          frame_number, video_frame, [speed_of_female, speed_of_female2], [dist_female, dist_female2])
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp-t1, angle, angle2, stim])
        cropped_image = cv_image[int(pos[0][1]) - 60:int(pos[0][1]) + 60, int(pos[0][0]) - 60:int(pos[0][0]) + 60]
        if cropped_image.shape == (120, 120):
