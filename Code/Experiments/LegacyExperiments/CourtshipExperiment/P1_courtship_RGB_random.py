...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp','female_angle','opto_stim'])
...
################### wait for 10 minutes with a black screen
win.color = (-1, -1, -1)
print('roshan')
###################
image, timestamp = image_methods.grab_image(camera, 'basler')
...
while (trials < 40):
    ...
    while (True):
        ...
        if frame_number == 0:
            ...
            print(trials)
            # if list(win.color) == [-1, 1, 1]:
            #     print('green+blue')
            #     dot_stim.fillColor = random.choice([(-1, -0.5, -0.5), (-1, -1, -1)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = 1
            # elif  list(win.color) == [1, 1, -1]:
            #     print('red+green')
            #     win.color = (1, 1, -1)
            #     dot_stim.fillColor = random.choice([(-1, -1, -1), (-0.5, -0.5, -1)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = 0
            ...
            #     print('greens')
            #     win.color = (-1, 1, -1)
            #     dot_stim.fillColor = random.choice([(-1, -1, -1), (-1, -0.5, -1)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = -3
            ...
            #     print('red+blue')
            #     win.color = (1, -1, 1)
            #     dot_stim.fillColor = random.choice([(-1, -1, -1), (-0.5, -1, -0.5)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = -1
            ...
            #     print('blue')
            #     win.color = (-1, -1, 1)
            #     dot_stim.fillColor = random.choice([(-1, -1, -1), (-1, -1, -0.5)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = -2
            ...
            # if 30>trials>=10:
            #     win.color = (-1, 1, -1)
            #     dot_stim.fillColor = random.choice([(-1, -0.5, -1), (-1, -1, -1)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = 1
            # else:
            #     win.color = (-1, -1, 1)
            #     dot_stim.fillColor = random.choice([(-1, -1, -1),(-1, -0.5, -0.5)])
            #     # dot_stim.radius = random.choice([1/5, 1/10, 1/15, 1/20, 1/25])
            #     # dot_stim.fillColor = random.choice([-1])
            #     dot_stim.lineColor = dot_stim.fillColor
            #     stim = 0
            # ard_ser.write(str.encode(input_str))
        elif frame_number >= 0 * framerate and frame_number < 10 * framerate:
            angle = 45-abs(90 - (speed_of_female*frame_number % 180))
            x, y = int(dist_female * np.cos((((fly_frame_ori + angle) / 180) * np.pi) + np.pi / 2) + pos[0][0]), int(
                dist_female * np.sin((((fly_frame_ori + angle) / 180) * np.pi) + np.pi / 2) + pos[0][1])
            x = (x - 512) / w_ratio + w_rem
            y = (y - 512) / h_ratio + h_rem
            dot_stim.pos = [[x, y]]
            dot_stim.draw()
        elif frame_number > 10 * framerate and frame_number < 30 * framerate:
            angle = 45-abs(90 - (speed_of_female*frame_number % 180))
            x, y = int(dist_female * np.cos((((fly_frame_ori + angle) / 180) * np.pi) + np.pi / 2) + pos[0][0]), int(
                dist_female * np.sin((((fly_frame_ori + angle) / 180) * np.pi) + np.pi / 2) + pos[0][1])
            x = (x - 512) / w_ratio + w_rem
            ...
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor), list(win.color)],on_time, off_time, frequency,
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp-t1, angle, stim])
        cropped_image = cv_image[int(pos[0][1]) - 60:int(pos[0][1]) + 60, int(pos[0][0]) - 60:int(pos[0][0]) + 60]
        if cropped_image.shape == (120, 120):
