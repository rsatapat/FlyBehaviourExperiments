...
while (trials < 80):
    ...
    # contrast = random.choice([10, 25, 75])
    ...
    # phase_difference = random.choice([0, 90, -90, 180])
    dot_amplitude = random.choice([45])
    # print(phase_difference)
    # dot_stim.fillColor = random.choice([-1, -0.2, -0.5, -0.8])
    # dot_stim.fillColor = random.choice([-1, -0.2])
    # Hz = random.choice([1, 4])
    # Hz = random.choice([4, 8])
    Hz = random.choice([10])
    phase = random.choice([0, 0.25, 0.5, 0.75])
    while (True):
        image, timestamp = image_methods.grab_image(camera, 'basler')
        cv_image = ROI(image, loc, size)
        ret, diff_img = cv2.threshold(cv_image, 60, 255, cv2.THRESH_BINARY)
        ...
        if frame_number == 0:
            ...
            # else:
            #     ard_ser.write(b'H')
        elif frame_number >= 0 * framerate and frame_number < 12 * framerate:
            stim = 1
            ...
            # wheel_stim.ori = wheel_stim.ori + Hz * 5 * random.random() * random.choice([1, -1])  ## for random noise stimulus
            optomotor_stim = 0
            dir = 0
            ...
            wheel_stim.draw()
            dot_stim.draw()
        elif frame_number >= 12 * framerate and frame_number < 24 * framerate:
            stim = 0
            angle = dot_amplitude-abs(2*dot_amplitude - (speed_of_female*frame_number % (4*dot_amplitude)))
            # angle = frame_number * 2 * speed_of_female % 360
            # angle = dot_amplitude * (np.sin((angle / 180) * np.pi))
            ...
            # pinwheel_angle = ((frame_number * 2 * speed_of_female)+phase_difference) % 360
            # pinwheel_angle = dot_amplitude * (np.sin((pinwheel_angle / 180) * np.pi))
            # wheel_stim.ori = pinwheel_angle * Hz
            #############
            # wheel_stim.ori = wheel_stim.ori + Hz * 5 * random.random() * random.choice([1, -1]) ## for random noise stimulus
            ########
            # if frame_number %int((dot_amplitude * 4)/speed_of_female) > int(((dot_amplitude * 4)/speed_of_female)* phase) - 2 and frame_number%int((dot_amplitude * 4)/speed_of_female) < int(((dot_amplitude * 4)/speed_of_female)* phase) + 8:
            #     wheel_stim.ori = wheel_stim.ori + dir * (360 * Hz) / (framerate * bars)
            #########
            wheel_stim.draw()
            dot_stim.draw()
        elif frame_number >= 24 * framerate:
            print('Stimulation off...')
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor), bars, contrast, Hz, phase],on_time, off_time, frequency,
