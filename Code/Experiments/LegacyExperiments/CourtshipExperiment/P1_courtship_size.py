...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp','female_size','opto_stim'])
...
while (trials < 20):
    ...
    frame_lost = 0
    dist_female = 150
    speed_of_female = random.choice([1])
    ...
    while (True):
        ...
        if frame_number == 0:
            ...
            x, y = int(dist_female * np.cos((((fly_frame_ori + 0) / 180) * np.pi) + np.pi / 2) + pos[0][0]), int(
                dist_female * np.sin((((fly_frame_ori +0) / 180) * np.pi) + np.pi / 2) + pos[0][1])
            x = (x - 512) / w_ratio + w_rem
            y = (y - 512) / h_ratio + h_rem
            dot_stim.pos = [[x, y]]
            angle = 60 - abs(90 - (speed_of_female * frame_number % 180))
            ...
            stimulus_inst = objects.stim_inst([stimulus, float(dot_stim.radius), list(dot_stim.fillColor)],on_time, off_time, frequency,
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp-t1, float(dot_stim.radius), stim])
        cropped_image = cv_image[int(pos[0][1]) - 60:int(pos[0][1]) + 60, int(pos[0][0]) - 60:int(pos[0][0]) + 60]
        if cropped_image.shape == (120, 120):
