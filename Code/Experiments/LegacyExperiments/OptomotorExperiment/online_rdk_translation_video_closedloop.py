...
while (trials < 30):
    ...
    speed = random.choice([0.005, 0.01, 0.02, 0.05])
    contrast = -1
    ...
    while (True):
        ...
        if frame_number == 0:
            ...
            dot_stim1.xys = np.add(dot_stim1.xys,
                                   np.tile(np.multiply(np.array([change_x, change_y]), dir), (N, 1)))
            ...
            dot_stim1.xys = np.add(dot_stim1.xys, np.tile(np.multiply(np.array([change_x, change_y]), dir), (N, 1)))
