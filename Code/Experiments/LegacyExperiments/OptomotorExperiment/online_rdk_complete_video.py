...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp', 'rotation', 'translation'])
...
while (trials < 30):
    ##parameters
    N = random.choice([225])
    # coherence = random.choice([0.5, 0.6, 0.75, 0.9, 1])
    coherence = random.choice([1])
    n1 = int(N * coherence)
    n2 = int(N * (1 - coherence))
    # stim_size = random.choice([0.02, 0.04, 0.1])
    stim_size = random.choice([0.1])
    # speed = random.choice([0.2, 0.5, 1, 2, 5])
    # speed = random.choice([5])
    # angle = speed
    # direction = random.choice([1, -1])
    speed = random.choice([1])
    contrast = -1
    # rod = random.choice([20])
    # rod = random.choice([0.01, 0.1, 0.25, 0.5, 0.75])
    rod = random.choice([0.1])
    rod = int(rod*N)
    print(trials, rod, coherence)
    # closedloop_gain = random.choice([0,0.2,0.5,1])
    closedloop_gain = random.choice([0.5, 1])
    closedloop_gain = 1

    dot_stim1 = psychopy.visual.ElementArrayStim(win, units='norm', fieldPos=(0.0, 0.0), fieldSize=(3.0, 3.0), fieldShape='circle', nElements=n1, sizes=stim_size,
    ...
    # dot_stim2 = psychopy.visual.ElementArrayStim(win, units='norm', fieldPos=(0.0, 0.0), fieldSize=(3.0, 3.0), fieldShape='circle', nElements=n2, sizes=stim_size,
    #                                              xys=None, rgbs=None, colors=contrast, colorSpace='rgb',
    #                                              opacities=1.0, depths=0, fieldDepth=0, oris=0, sfs=1.0, contrs=1, phases=0, elementTex='circle',
    #                                              elementMask='circle', texRes=48, interpolate=True, name=None, autoLog=None, maskParams=None)
    ...
    while (True):
        ...
        ######################
        theta = np.radians(fly_turn)  # could use radians if you like
        rot_mat1 = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
        rot_mat2 = np.array([[np.cos(-theta), -np.sin(-theta)], [np.sin(-theta), np.cos(-theta)]])
        ######################
        ...
        # change_x = last_stim_pos[0][0] - x
        # change_y = last_stim_pos[0][1] - y
        #######################
        print(fly_frame_ori%360)
        change_x = 0.01 * np.cos(np.radians((fly_frame_ori%360)+ 90))
        change_y = 0.01 * np.sin(np.radians((fly_frame_ori%360)+ 90))
        #######################
        if frame_number == 0:
            ...
            dot_stim1.xys[index1] = np.stack((np.subtract(2 * np.random.random_sample(rod), 1), np.subtract(2 * np.random.random_sample(rod), 1)), axis=1)
            # dot_stim1.xys = np.dot(dot_stim1.xys, rot_mat1)
            # dot_stim2.xys = np.dot(dot_stim2.xys, rot_mat2)
            dot_stim1.xys = np.add(dot_stim1.xys,
                                   np.tile(np.multiply(np.array([change_x, change_y]), closedloop_gain), (N, 1)))
            ...
            # dot_stim2.xys = np.dot(dot_stim2.xys, rot_mat2)
            dot_stim1.xys = np.add(dot_stim1.xys,
                                   np.tile(np.multiply(np.array([change_x, change_y]), -1*closedloop_gain), (N, 1)))
            ...
            if n2 == 0:
                ...
                #                                   np.subtract(2*np.random.random_sample(int(rod*(n2/n1))), 1)), axis=1)
            dot_stim1.xys[index1] = np.stack((np.subtract(2*np.random.random_sample(rod), 1), np.subtract(2*np.random.random_sample(rod), 1)), axis=1)
            ## exchnage the rotation matrices to flip the direction
            # dot_stim1.xys = np.dot(dot_stim1.xys, rot_mat2)
            # dot_stim2.xys = np.dot(dot_stim2.xys, rot_mat1)
            dot_stim1.xys = np.add(dot_stim1.xys, np.tile(np.multiply(np.array([change_x, change_y]), -1*closedloop_gain), (N, 1)))
        elif frame_number >= 20 * framerate:
            stimulus_inst = objects.stim_inst([stimulus, coherence, rod, closedloop_gain], speed, N, 1,
