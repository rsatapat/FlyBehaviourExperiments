...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp', 'direction', 'bar'])
...
while (trials < 30):
    ...
    stimulus_image_bar = r'{}/{}'.format(r'D:\Roshan\Project\PythonCodes\Stimulus\Generate stimulus', 'bar_'+str(100)+'_'+str(12)+'.png')

    # stimulus_image = random.choice([stimulus_image_pinwheel, stimulus_image_natural, stimulus_image_random_dots])
    ...
    bar_stim = psychopy.visual.ImageStim(
        win=win,
        image=stimulus_image_bar,
        mask='circle',
        pos=(0, 0),
        ori=0,  # 0 is vertical,positive values are rotated clockwise
        size=1.5,
    ...
    while (True):
        ...
        if frame_number == 0:
            ...
            dir = direction
            bar_stim.ori = (-fly_frame_ori - 45) + 180 + (dir * 45 * -1)
            bar = 1
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            bar_stim.draw()
        elif frame_number >= 20 * framerate and frame_number < 25 * framerate:
            dir = 0
            bar = 0
            wheel_stim.draw()
        elif frame_number >= 25 * framerate and frame_number < 30 * framerate:
            if frame_number == 25*framerate:
                bar_stim.ori = (-fly_frame_ori - 45) + 180
            dir = 0
            bar_stim.ori = bar_stim.ori + direction*(angle/4)
            bar = -1
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            bar_stim.draw()
        elif frame_number >= 30 * framerate and frame_number < 35 * framerate:
            dir = (-1) * direction
            bar = 0
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
        elif frame_number >= 35 * framerate and frame_number < 40 * framerate:
            dir = (-1) * direction
            bar_stim.ori = (-fly_frame_ori - 45) + 180 + (dir * 45)
            bar = -1
            wheel_stim.ori = (wheel_stim.ori + dir * angle - closedloop_gain * fly_turn) % 360
            wheel_stim.draw()
            bar_stim.draw()
        elif frame_number >= 40 * framerate:
            stimulus_inst = objects.stim_inst([stimulus, stimulus_image, angle/2], direction, bars, Hz,
        ...
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp  - t1, dir, bar])
