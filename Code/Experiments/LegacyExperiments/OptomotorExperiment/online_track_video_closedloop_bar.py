...
writer_csv.writerow(['pos_x', 'pos_y', 'ori', 'timestamp', 'direction', 'bar_angle'])
...
while (trials < 60):
    ...
    stimulus = random.choice(['sine_pinwheel_new127'])
    print(stimulus)
    # bars=5
    # contrast = random.choice([10, 25, 50, 100])
    contrast = random.choice([25, 100])
    ...
    ########################################
    ## add a bar on top of the pinwheel
    bar_stimulus_image = os.path.join(r'D:\Roshan\Project\PythonCodes\Stimulus\Generate stimulus', 'transparent_bar_100_12.png')
    bar_stim = psychopy.visual.ImageStim(
        win=win,
        image=bar_stimulus_image,
        mask='circle',
        pos=(0, 0),
        ori=0,  # 0 is vertical,positive values are rotated clockwise
        size=1.5,
    )
    bar_stim.autoDraw = False
    bar_closedloop_gain = 1
    bar_stimulus_on_or_not = random.choice([1])
    print('bar stim :' + '  ' + str(bar_stimulus_on_or_not))
    bar_bias = random.choice([30, 0, -30])
    print(bar_bias)
    ########################################
    ...
    wheel_stim.ori = 180 - fly_frame_ori_last
    # main loop
    ...
    while (True):
        ...
        if frame_number == 0:
            ...
            dir = 0
        elif frame_number >= 1 * framerate and frame_number < 2000 * framerate:
            dir = direction
            ...
            stimulus_inst = objects.stim_inst([stimulus, stimulus_image, list(win.color), bar_stimulus_on_or_not], direction, bars, Hz,
        ...
        bar_stim.ori = bar_bias - (fly_frame_ori%360)
        wheel_stim.draw()
        if bar_stimulus_on_or_not:
            bar_stim.draw()
        win.flip()  # slowest part of the algorithm,takes ~10ms
        writer_csv.writerow([pos[0][0], pos[0][1], fly_frame_ori, timestamp  - t1, dir, bar_stim.ori])
        cropped_image = cv_image[int(pos[0][1]) - 30:int(pos[0][1]) + 30, int(pos[0][0]) - 30:int(pos[0][0]) + 30]
        cropped_image_new = cv2.line(cropped_image, (30,30),\
